...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL','postgresql:///phoodie')
...
def searchresults():
    ...
    if location:
        ...
        response = requests.get(
            API_BASE_SEARCH, params=parameters, headers=resp_header)
        data = response.json()
...
def business_detail(business_id):
    response = requests.get(
        f"https://api.yelp.com/v3/businesses/{business_id}", headers=resp_header)
    response2 = requests.get(
        f"https://api.yelp.com/v3/businesses/{business_id}/reviews", headers=resp_header)
    data = response.json()
    ...
    if g.user:
        ...
        user_id = g.user.id
        faveslookup = Favorite.query.filter(
            Favorite.user_id == user_id, Favorite.business_id == business_id).all()
        faveID = Favorite.query.filter_by(business_id=business_id).all()
        myfaves = Favorite.query.filter_by(user_id=user_id).all()
        faves_businessID = [f.business_id for f in myfaves]
        favesIDlookup = [f.id for f in faveslookup]

        return render_template('businessdetails.html', data=data, data2=data2, myfaves=myfaves, businessid=businessid, faves_businessID=faves_businessID, faveID=faveID, user_id=user_id,  favesIDlookup=favesIDlookup)

    else:
        return render_template('businessdetails.html', data=data, data2=data2, businessid=businessid)
...
def follow_users(following_id):
    ...
    new_follow = Follows(user_following_id=user_following_id,
                         user_being_followed_id=user_being_followed_id)
